...
// ScanService defines the subset of ScanService methods used by handlers
type ScanService interface {
	StartScan(assetID string, scanType model.ScanType) (*model.ScanJob, error)
	GetScanJob(jobID string) (*model.ScanJob, error)
	GetScanResults(jobID string) (interface{}, error)
	ListScanJobs(assetID string) ([]*model.ScanJob, error)
	GetAssetSubdomains(assetID string) ([]*model.Subdomain, error)
	GetAssetDNSRecords(assetID string) ([]*model.DNSRecord, error)
	GetAssetWHOIS(assetID string) (*model.WHOISRecord, error)
	GetAssetAllScanResults(assetID string) (map[string]interface{}, error)
	DemoSyncVsAsync(assetID string) error
}

func NewScanHandler(scanService ScanService) *ScanHandler {
	return &ScanHandler{
		scanService:  scanService,
...
func (h *ScanHandler) StartScan(w http.ResponseWriter, r *http.Request) {
	...
	assetID := h.pathProvider.Get(r, "id")
	if assetID == "" {
