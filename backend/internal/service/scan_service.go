...
type ScanService struct {
	...
	dnsScanner  interface {
		Scan(*model.Asset) ([]*model.DNSRecord, error)
	}
	whoisScanner interface {
		Scan(*model.Asset) (*model.WHOISRecord, error)
	}
	subdomainScanner interface {
		Scan(*model.Asset, context.Context) ([]*model.Subdomain, error)
	}
	ipScanner interface {
		Scan(*model.Asset) ([]*model.IPResult, error)
	}
...
// performIPScan executes IP geolocation and ASN lookups for IP assets
func (s *ScanService) performIPScan(asset *model.Asset, job *model.ScanJob) error {
	// Only valid for IP assets
	if asset.Type != model.TypeIP {
		return fmt.Errorf("asset is not an IP type: %s", asset.ID)
	}

	// The IP scanner returns one or more IPResult entries
	results, err := s.ipScanner.Scan(asset)
	if err != nil {
	...
	for _, r := range results {
		...
		if err := s.scanStorage.CreateIPResult(r); err != nil {
			return fmt.Errorf("failed to save ip result: %w", err)
		}
	}
...
// performPortScan executes an active port scan (safeguarded) and saves results
func (s *ScanService) performPortScan(asset *model.Asset, job *model.ScanJob) error {
	// Only proceed for IP or domain assets
	if asset.Type != model.TypeIP && asset.Type != model.TypeDomain {
		return fmt.Errorf("asset is not an IP or domain type: %s", asset.ID)
	}

	// Run the port scanner (returns scanner-level results)
	hostResults, err := s.portScanner.Scan(asset)
	if err != nil {
	...
	for _, hostResult := range hostResults {
		pr := &model.PortScanResult{
			...
			ClosedPorts:    hostResult.ClosedPorts,
			TotalScanned:   hostResult.TotalScanned,
			ScanDurationMs: int(hostResult.ScanDuration),
			CreatedAt:      hostResult.CreatedAt,
		}

		open := make([]model.OpenPort, 0, len(hostResult.OpenPorts))
		for _, p := range hostResult.OpenPorts {
		...
		if err := s.scanStorage.CreatePortScanResult(pr); err != nil {
