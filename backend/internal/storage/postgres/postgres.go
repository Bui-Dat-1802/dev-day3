...
// CreateIPResult inserts a new IP result into the database
func (p *PostgresStorage) CreateIPResult(result *model.IPResult) error {
	query := `
		INSERT INTO ip_results (
			id, asset_id, scan_job_id, ip_address,
			country, country_code, city, region, latitude, longitude,
			isp, org, as_number, as_name, as_description, reverse_dns, created_at
	...
	_, err := p.db.Exec(
		...
		result.Geolocation.CountryCode,
	...
	if err != nil {
		return fmt.Errorf("failed to create ip result: %w", err)
	}
	return nil
}

// GetIPResultsByAsset retrieves all IP results for an asset
func (p *PostgresStorage) GetIPResultsByAsset(assetID string) ([]*model.IPResult, error) {
	query := `
		SELECT id, asset_id, scan_job_id, ip_address,
			   country, country_code, city, region, latitude, longitude,
			   isp, org, as_number, as_name, as_description, reverse_dns, created_at
		FROM ip_results
		WHERE asset_id = $1
		ORDER BY created_at DESC
	`
	return p.queryIPResults(query, assetID)
}

// GetIPResultsByScan retrieves all IP results discovered by a scan
func (p *PostgresStorage) GetIPResultsByScan(scanJobID string) ([]*model.IPResult, error) {
	query := `
		SELECT id, asset_id, scan_job_id, ip_address,
			   country, country_code, city, region, latitude, longitude,
			   isp, org, as_number, as_name, as_description, reverse_dns, created_at
		...
		ORDER BY created_at DESC
	`
	return p.queryIPResults(query, scanJobID)
}

// queryIPResults helper
func (p *PostgresStorage) queryIPResults(query string, arg string) ([]*model.IPResult, error) {
	rows, err := p.db.Query(query, arg)
	if err != nil {
		return nil, fmt.Errorf("failed to query ip results: %w", err)
	}
	defer rows.Close()

	results := []*model.IPResult{}
	for rows.Next() {
		r := &model.IPResult{}
		var asNumber sql.NullInt64
		var asName sql.NullString
		var asDesc sql.NullString
		...
		if isp.Valid {
			r.Geolocation.ISP = isp.String
		}
		if org.Valid {
			r.Geolocation.Org = org.String
		}
		if asNumber.Valid {
			r.ASN.Number = int(asNumber.Int64)
		}
		if asName.Valid {
			r.ASN.Name = asName.String
		}
		if asDesc.Valid {
			r.ASN.Description = asDesc.String
		}
...
// CreatePortScanResult inserts a new port scan result into the database
func (p *PostgresStorage) CreatePortScanResult(result *model.PortScanResult) error {
	// open_ports stored as JSONB
	openPortsJSON, err := json.Marshal(result.OpenPorts)
	if err != nil {
		return fmt.Errorf("failed to marshal open_ports: %w", err)
	}

	query := `
		INSERT INTO port_scan_results (
			id, asset_id, scan_job_id, ip_address, open_ports, closed_ports, total_scanned, scan_duration_ms, created_at
		) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
	`
...
// GetPortScanResultsByAsset retrieves port scan results for an asset
func (p *PostgresStorage) GetPortScanResultsByAsset(assetID string) ([]*model.PortScanResult, error) {
	query := `
		SELECT id, asset_id, scan_job_id, ip_address, open_ports, closed_ports, total_scanned, scan_duration_ms, created_at
		FROM port_scan_results
		WHERE asset_id = $1
		ORDER BY created_at DESC
	`
	return p.queryPortResults(query, assetID)
}

// GetPortScanResultsByScan retrieves port scan results for a scan job
func (p *PostgresStorage) GetPortScanResultsByScan(scanJobID string) ([]*model.PortScanResult, error) {
	query := `
		...
		ORDER BY created_at DESC
	`
	return p.queryPortResults(query, scanJobID)
}

// queryPortResults helper
func (p *PostgresStorage) queryPortResults(query string, arg string) ([]*model.PortScanResult, error) {
	rows, err := p.db.Query(query, arg)
	if err != nil {
	...
	for rows.Next() {
		...
		// Unmarshal open ports JSON
		var open []model.OpenPort
		if len(openPortsBytes) > 0 {
			if err := json.Unmarshal(openPortsBytes, &open); err != nil {
				return nil, fmt.Errorf("failed to unmarshal open_ports: %w", err)
			}
		}
		r.OpenPorts = open
